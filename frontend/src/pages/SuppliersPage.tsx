...
import { Plus, Search, Pencil, Trash2, ShoppingCart, PackageCheck } from 'lucide-react';
import { db } from '@/db';
...
export function SuppliersPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editing) {
      ...
      await logAction({ action: 'modification', entity: 'fournisseur', entityId: editing.id, entityName: form.name });
    } else {
      const id = generateId();
    ...
    <div className="space-y-4">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input id="sname" label="Nom" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
          <Input id="sphone" label="Téléphone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} required />
          <Input id="saddr" label="Adresse" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
      ...
      >
        <div className="space-y-6">
          <div>
            <h4 className="text-sm font-semibold mb-3">Nouvelle commande</h4>
                ...
                <div key={i} className="flex gap-2 items-end">
                  <div className="flex-1">
                    <select
                      ...
                      value={op.productId}
                      onChange={(e) => updateOrderProduct(i, 'productId', e.target.value)}
                      required
                    >
                  ...
                  <input
                    ...
                    value={op.quantity}
                    onChange={(e) => updateOrderProduct(i, 'quantity', Number(e.target.value))}
                    placeholder="Qté"
                    required
                  />
                  <input
                    ...
                    value={op.unitPrice}
                    onChange={(e) => updateOrderProduct(i, 'unitPrice', Number(e.target.value))}
                    placeholder="Prix"
                    required
                  />
          ...
          {supplierOrders.length > 0 && (
            <div className="border-t border-border pt-4">
              <h4 className="text-sm font-semibold mb-3">Historique des commandes</h4>
