...
export function CustomersPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editing) {
      ...
      await logAction({ action: 'modification', entity: 'client', entityId: editing.id, entityName: form.name });
    } else {
      const id = generateId();
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Supprimer ce client ?')) {
      ...
      await db.creditTransactions.where('customerId').equals(id).delete();
